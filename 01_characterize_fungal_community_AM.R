...
#################################################################################
#                               Main workflow                                   #
#  Subset fungal community data to just AM in WFDP. Merge with exploration type #
...
# This was pared down to just the trees that we have both community and host tree trait measurements for 
# which results in 28 trees 
...
# number of samples - 28 host trees 
...
WFDP_AM_tax <- select(WFDP_AM_tax, X, Family, Genus, Species)
...
# Save tax table for function searching 

write.csv(WFDP_AM_tax, "~/Dropbox/WSU/WFDP_Chapter_3_Project/Fungal_Communities/WFDP_AM_subset_tax.csv")


### !! STOP: Need to figure out functional format first !! ## 
